...
function checkLocalStorage(): void {
  const storedValue = localStorage.getItem('languageSelected');
  if (storedValue) {
...
const languagedropdown: HTMLSelectElement = document.getElementById('languageselect_dropdown') as HTMLSelectElement;
let language: string = languagedropdown.options[languagedropdown.selectedIndex].value;

function languageChange(language: string): void {
  ...
  fetch(`../JSON/${language}.json`)
      ...
      .then(language => {
          elementIds.forEach(id => {
              ...
              if (element) {
                  element.textContent = language?.[id];
              }
          });
      })
