...
import { baseImageUrl,baseUrl } from "../script/home.js";
import { setNavbarProfiles } from "../script/home.js";

window.onload = () => {
    setNavbarProfiles();
    document.getElementById('advancedsearchbox').value = localStorage.getItem('searchKeyword');
    searchMoviesAndShows(localStorage.getItem('searchKeyword'));
}

document.getElementById('advancedsearchbutton').addEventListener('click',() => {
...
const searchMoviesAndShows = (searchKeyword) => {
    ...
    let content = [];
  fetch(
    baseUrl+`search/multi?query=${searchKeyword}&include_adult=false&language=en-US&page=1`,
    getOptions
  )
    ...
    .then((response) => {
      ...
      content.forEach(async (item) => {
        ...
        try {
            const response = await fetch(`https://api.themoviedb.org/3/movie/${itemId}/videos?language=en-US`, getOptions);
        ...
        console.log(`https://www.youtube.com/embed/${videoKey}?autoplay=1&mute=1&loop=1`);
